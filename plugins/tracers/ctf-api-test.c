...
static const char cpuusage_metadata_event_header[] = "event {\n\
	name = cpuusage;\n\
	id = %d;\n\
	fields := struct {\n\
...
void
TracerInit (tracer_struct * tracer)
{
...
CTFEventHeader (event_id id)
{
  ...
  GstClockTime elapsed =
      GST_CLOCK_DIFF (tracer.tracer_start_time, gst_util_get_timestamp ());
  elapsed = elapsed / 1000;
  timestamp = elapsed;
#if 0
  GST_ERROR ("event");
  g_printf ("%" GST_TIME_FORMAT " event %d\n", GST_TIME_ARGS (elapsed * 1000),
      elapsed);
...
void
CTFMetadataGenerate (int major, int minor, char *UUID, int byte_order)
{
  /* Convert UUID */
...
void
CTFMetadataAddEvent (const char *metadata_event, event_id id)
{
  fprintf (tracer.metadata, metadata_event, id);
}

void
CTFNewCpuUsageEvent (int16_t event_id, int32_t num, int64_t usage)
{
  FILE *fd;
...
void
CTFNewProcTimeEvent (int16_t event_id, char *element, int64_t time)
{
  FILE *fd;
...
void
CTFNewFPSEvent (int16_t event_id, char *element, int32_t fps)
{
  FILE *fd;
...
void
CTFNewTimerInitEvent (int16_t event_id, int32_t timer)
{
  //~ int size = strlen(msg);
  /* Add event ID */
  int32_t unknown;
...
main (int argc, char *argv[])
{
  ...
  //~ sleep(1);
  CTFNewProcTimeEvent (PROCTIME_EVENT_ID,
      "identity0123456789123456789123456789123456789123456789123456789", 1000);
  //~ sleep(1);
